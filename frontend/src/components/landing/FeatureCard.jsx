...
/**
 * FEATURE CARD - Obsidian Dark Edition
 * @param {ReactNode} icon - Icon component from react-icons
 * @param {string} title - Heading text
 * @param {string} desc - Description text
 * @param {string} accentColor - Tailwind color class
 */
export default function FeatureCard({ icon, title, desc, accentColor = "indigo" }) {
  // Adjusted color map for High-Contrast Dark Mode
  const colorMap = {
    indigo: { glow: "rgba(99, 102, 241, 0.15)", text: "text-indigo-400", border: "border-indigo-500/20" },
    ...
    rose: { glow: "rgba(244, 63, 94, 0.15)", text: "text-rose-400", border: "border-rose-500/20" },
  };
  ...
  return (
    <motion.div
      ...
      className="
        relative p-8 rounded-[2.5rem] overflow-hidden
    ...
    >
      ...
      <div
        ...
        transition-all group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(99,102,241,0.2)]
      ...
      {/* Bottom Glow Line */}
      <div 
        className="h-1 w-0 group-hover:w-12 rounded-full mt-8 transition-all duration-500" 
