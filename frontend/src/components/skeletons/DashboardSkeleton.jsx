...
export default function DashboardSkeleton() {
  return (
    <div className="animate-fadeIn pb-12 px-4 lg:px-8 pt-6 transition-colors duration-500"
         style={{ backgroundColor: "var(--bg-primary)" }}>
      
      {/* Header Skeleton */}
      <div className="mb-10">
        <Skeleton className="h-12 w-64 rounded-2xl mb-4" floating={true} />
        <Skeleton className="h-4 w-96 rounded-xl opacity-30" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        {[1, 2, 3, 4].map((i) => (
          <div 
            key={i} 
            className="p-6 rounded-[2rem] border h-64 flex flex-col shadow-2xl"
            style={{ backgroundColor: "var(--bg-card)", borderColor: "var(--border-color)" }}
          >
            <Skeleton className="h-2 w-16 mb-6 opacity-20" />
            <div className="flex-1 flex flex-col justify-center">
              <Skeleton className="h-16 w-24 rounded-2xl mb-4" floating={true} />
              <Skeleton className="h-3 w-3/4 rounded-md opacity-30" />
            </div>
          </div>
        ))}
      </div>

      {/* Action Buttons Skeleton (Quick Actions) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <Skeleton className="h-20 rounded-2xl opacity-50" />
        <Skeleton className="h-20 rounded-2xl opacity-50" />
        <Skeleton className="h-20 rounded-2xl opacity-50" />
      </div>

      {/* Content Area Skeleton (Charts/Activity) */}
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        <div className="p-2 rounded-[2.5rem] border" style={{ borderColor: "var(--border-color)", backgroundColor: "var(--bg-card)" }}>
          <Skeleton className="h-96 rounded-[2.2rem]" floating={true} />
        </div>
        <div className="p-2 rounded-[2.5rem] border" style={{ borderColor: "var(--border-color)", backgroundColor: "var(--bg-card)" }}>
          <Skeleton className="h-96 rounded-[2.2rem]" />
        </div>
      </div>
    </div>
  );
