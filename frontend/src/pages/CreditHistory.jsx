  ...
  if (!logs) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen" style={{ backgroundColor: "var(--bg-primary)" }}>
        <FiLoader className="w-10 h-10 text-[var(--accent)] animate-spin mb-4" />
        <p className="text-[var(--text-secondary)] font-bold uppercase tracking-widest text-xs">Syncing Ledger...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen transition-colors duration-300 font-sans selection:bg-indigo-500/30" 
      ...
      <div className="max-w-4xl mx-auto p-6 md:p-10 pt-32">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12">
          ...
          <div className="hidden md:flex items-center gap-2 px-4 py-2 border rounded-2xl shadow-sm text-xs font-bold uppercase tracking-tight"
        ...
        {logs.length === 0 ? (
          <div className="text-center py-20 card border-[var(--border-color)]">
            <div className="w-20 h-20 rounded-3xl flex items-center justify-center mx-auto mb-5 border text-[var(--text-secondary)] opacity-20"
                 style={{ backgroundColor: "var(--bg-primary)", borderColor: "var(--border-color)" }}>
              <FiClock size={40} />
            </div>
            <p className="font-black text-xl" style={{ color: "var(--text-primary)" }}>No activity found</p>
            <p className="text-sm mt-1" style={{ color: "var(--text-secondary)" }}>Your transaction history will appear here.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {logs.map((log) => (
              <div
                key={log._id}
                className="group relative flex justify-between items-center transition-all duration-300 hover:translate-x-1 p-5 md:p-6 rounded-[28px] border"
                style={{ 
                  backgroundColor: "var(--bg-card)", 
                  borderColor: log.value > 0 ? "rgba(16, 185, 129, 0.2)" : "var(--border-color)" 
                }}
              >
                {/* Visual Status Indicator (Right Bar) */}
                <span className={`absolute top-1/2 -right-1 translate-y-[-50%] w-1.5 h-10 rounded-l-full ${
                    log.value > 0 ? "bg-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.5)]" : "bg-slate-700"
                }`}></span>

                <div className="flex items-center gap-5">
                  <div className={`shrink-0 w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${
                    log.value > 0 ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400"
                  }`}>
                    {log.value > 0 ? <FiArrowUpRight size={22} /> : <FiArrowDownLeft size={22} />}
                  </div>

                  <div>
                    <p className="text-lg font-bold tracking-tight capitalize leading-tight" style={{ color: "var(--text-primary)" }}>
                      {log.source.replace("_", " ")}
                    </p>
                    <div className="flex flex-wrap items-center gap-3 mt-1.5">
                        <p className="text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md"
                        ...
                        {log.meta?.questionId && (
                            <div className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md border font-bold"
                                ...
                                <FiHash size={10} className="opacity-70" />
                                {log.meta.questionId.slice(-6).toUpperCase()}
                            </div>
                        )}
                    </div>
                ...
                <div className="flex items-center gap-5">
                    <div className={`text-xl font-black tracking-tighter text-right ${log.value > 0 ? "text-emerald-400" : "text-rose-400"}`}>
                      <span className="text-[10px] block opacity-40 uppercase tracking-tighter -mb-1 font-bold">
                        {log.value > 0 ? "Earned" : "Spent"}
                      </span>
                      {log.value > 0 ? "+" : ""}
                    ...
                    <FiChevronRight className="hidden md:block group-hover:text-white transition-colors opacity-20 group-hover:opacity-100" />
                </div>
              </div>
            ))}
          </div>
